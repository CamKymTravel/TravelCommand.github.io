...
export function buildJourneyMapModel(state, years = ['all'], options = {}) {
  ...
  return {
    years: selectedYears,
    availableYears: availableTravelYears(state.itinerary || [], journeyStartDate, { minimum: 30 }),
    journeyStartDate,
    stays: itinerary.map((entry, index) => {
      ...
      id: entry.id,
      name: entry.name,
      country: (['cruise','motorhome','rv'].includes(String(entry.travelType||'').toLowerCase()) ? (entry.startCountry || entry.country || '') : (entry.country || entry.startCountry || '')) ,
