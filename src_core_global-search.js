...
export function searchCanonicalState(state,query,{limit=20,currentDate=null}={}){const normalized=comparisonText(query);if(!normalized)return[];const tokens=normalized.split(/\s+/).filter(Boolean);const results=[];const itineraryById=new Map((state.itinerary||[]).map(item=>[item.id,item]));const today=currentDate?toISODate(currentDate):null;for(const [collection,screenLabel] of SEARCHABLE_COLLECTIONS){for(const record of state[collection]||[]){if(collection==='calendarEvents'&&record.reservationId)continue;const repairCost=(collection==='expenses'||collection==='reservations')&&record.needsBudgetRepair===true;const linked=!repairCost&&record.itineraryId?itineraryById.get(record.itineraryId):null;if(collection==='journeyHistory'&&(!linked||(today&&toISODate(linked.endDate)>=today)))continue;const haystack=searchableText(record,linked);if(!tokens.every(token=>haystack.includes(token)))continue;results.push({id:record.id,collection,screenLabel,title:collection==='journeyHistory'&&typeof linked?.name==='string'&&linked.name.trim()?linked.name:resultTitle(record,screenLabel),subtitle:firstText((collection==='itinerary'&&['cruise','motorhome','rv'].includes(String(record.travelType||'').toLowerCase()))?(record.startCountry||record.country):record.country,record.notes,record.note,record.description),dateContext:resultDateContext(record,linked),needsBudgetRepair:repairCost,modifiedAt:record.modifiedAt||record.createdAt||null});}}return results.sort((a,b)=>String(b.modifiedAt||'').localeCompare(String(a.modifiedAt||''))||a.title.localeCompare(b.title)).slice(0,Math.max(0,Number(limit)||0));}
