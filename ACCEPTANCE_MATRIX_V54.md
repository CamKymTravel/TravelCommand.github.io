# Travel Command Centre V1 — V54 Acceptance Matrix
...
- The target iPad interaction rule is effective final cascade, not merely an earlier 44px declaration.
- `Reservations` flight-scope tiles must finish at **44px minimum height** after all later acceptance rules.
- `Itinerary` Forward Coverage switches must finish at **44px minimum height** after all later acceptance rules.
- The verifier must fail if a later same-layer rule shrinks either protected control below 44px, even when an earlier 44px hardening marker is still present.


## V54 reservation-scale and destination-context protection
- Reservations Health duplicate grouping must preserve the established rule: same normalized type/title/date is duplicate when times match, and a date-only record acts as a wildcard connecting timed copies on that date; two explicit different times remain legitimate.
- The health/model implementation must use the indexed duplicate-key path rather than restoring the all-pairs O(n²) scan.
- New or edited **Standard** itinerary stays require a non-blank Country. This is needed for the country header, Country Quick Look and Where’s-the-toilet destination-language context.
- Cruise and Motorhome/RV trips continue to use **Starting Country** for departure-language/header context; do not replace that rule with the Standard Country requirement.
- Older canonical data with a blank Standard Country must remain loadable. App Health should flag it for repair rather than destructive migration or silent substitution.
