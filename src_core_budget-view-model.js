...
import { forecastAnnualSpend, remainingBudget, isDestinationBudgetUsable, annualBudgetForYear, budgetPeriodMetrics, budgetPeriodForYear } from './src_core_budget.js';
...
function annualMonthlyHistory(state, currentYear, currentDate) {
  ...
  const years = new Set([Number(currentYear), ...Object.keys(state.settings?.annualBudgetsAUD || {}).map(Number).filter(Number.isFinite)]);
  for (const record of records) { const year = Number(record.date.slice(0, 4)); if (Number.isFinite(year)) years.add(year); }
  const sortedYears = [...years].sort((a, b) => b - a);
  const histories = sortedYears.map(year => {
    const months = Array.from({ length:12 }, (_, index) => ({ month:index + 1, amountAUD:0 }));
    for (const record of records) { if (Number(record.date.slice(0, 4)) !== year) continue; const month = Number(record.date.slice(5, 7)); if (month >= 1 && month <= 12) months[month - 1].amountAUD += Number(record.audAmount || 0); }
    ...
    const budgetAUD = annualBudgetForYear(state.settings, year);
    const period = budgetPeriodForYear(state.settings, year);
    return { year, months, spentAUD, recordedMonths, averageRecordedMonthAUD:recordedMonths ? spentAUD / recordedMonths : 0, peakMonth:peak?.month ?? null, peakMonthAUD:peak?.amountAUD ?? 0, monthlyTargetAUD:period.activeMonths ? budgetAUD / period.activeMonths : 0, budgetPositionAUD:budgetAUD - spentAUD, budgetAUD, periodStart:period.start, activeMonths:period.activeMonths };
  });
...
function recentExpenses(expenses, today, limit = 8) {
  ...
  return records
    .map(record => {
      ...
      return {
        id:record.id,
        budgetScope:record.budgetScope === 'annual' ? 'annual' : 'destination',
...
export function buildBudgetViewModel(state, currentDate, options = {}) {
  ...
  const annualTime = budgetPeriodMetrics(state.settings, currentDate);
  const annual = annualLedger(state, annualTime.year);
  const annualBudgetAUD = annualBudgetForYear(state.settings, annualTime.year);
  const today = toISODate(currentDate);
  ...
  // The Budget Reservations panel is a read-only view of bookings linked to
  // this exact stay, regardless of which budget ultimately pays the cost.
  // Every real cost rolls into the annual travel total. Reservation budgetScope
  // controls only whether that booking also consumes this stay's Destination Budget.
  const linkedReservations = stay
    ? (state.reservations || []).filter(record => !record.needsBudgetRepair && record.status !== 'to-book' && record.itineraryId === stay.id)
    : [];
  ...
  return {
    currentDestination: stay ? {
      id:stay.id,
      name:stay.name,
      country:displayCountryForStay(stay),
    ...
    reservations: linkedReservations.map(record => ({
      ...
      originalAmount:Number(record.originalAmount || 0),
      audAmount:Number(record.audAmount || 0),
      budgetScope:record.budgetScope || (record.type === 'ticket' ? 'destination' : 'annual')
    })),
    categories: {
    ...
    monthlyHistory:annualMonthlyHistory(state, annualTime.year, currentDate),
