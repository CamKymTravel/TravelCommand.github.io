...
function renderHealth(stateService,currentDate,host){
  ...
  const grid=node('div','settings-health-grid');for(const item of model.checks){
    ...
    grid.append(card);
  }panel.append(grid);if(host)makeExpandableCard(panel,{host,title:'App Health',tone:model.status==='verified'?'green':model.status==='needs-setup'?'gold':'red'});return panel;
}
