...
function openGeneralEditor({stateService,host}) {
  ...
  const modal=createModal({title:'Travel & Budget Defaults',body,className:'tone-blue',actions:[
...
function openSchengenEditor({stateService,host}) {
  ...
  const modal=createModal({title:'Schengen Status · Manual / Offline',body,className:'tone-teal',actions:[{label:'Cancel',onClick:d=>d.close()},{label:'Save',onClick:d=>{try{stateService.commit(draft=>saveSchengenSettingsDraft(draft,capture()));if(d.isConnected&&d.open)d.close();}catch(err){error.textContent=err.message;}}}]});modalHost(host,modal);
}
...
function openPinEditor({stateService,host}) {
  ...
  const modal=createModal({title:enabled?'Change PIN':'Set PIN',body,className:'tone-indigo',actions:[
...
function openDisablePin({stateService,host}) {
  ...
  const modal=createModal({title:'Disable PIN',body,className:'tone-indigo',actions:[
    {label:'Cancel',onClick:dialog=>dialog.close()},
...
function renderHealth(stateService,currentDate,host){
  ...
  const run=node('button',`settings-health-run settings-health-run-${model.status}`,model.status==='verified'?'✓ APP HEALTH VERIFIED':'＋ CHECK THE WHOLE APP'); run.type='button'; run.addEventListener('click',async()=>{const focusBeforeCheck=captureLocalFocus();run.textContent='CHECKING…';run.disabled=true;try{await stateService.auditVaultAssets?.();}catch{}setTimeout(()=>{if(!panel.isConnected)return;const replacement=renderHealth(stateService,currentDate,host);panel.replaceWith(replacement);restoreLocalFocus(focusBeforeCheck,{fallbackSelector:'.settings-health-run'});},120);}); panel.append(run);
  const grid=node('div','settings-health-grid');for(const item of model.checks){
    ...
    grid.append(card);
  }panel.append(grid);if(host)makeExpandableCard(panel,{host,title:'App Health',tone:'green'});return panel;
}
