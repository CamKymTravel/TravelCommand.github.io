...
function renderHealth(stateService,currentDate,host){
  ...
  const HEALTH_CARD_TONES=Object.freeze({
    ...
    'Reservations':'copper',
    'Calendar':'teal',
    'Journey History':'green',
    'Checklist':'blue',
    'The Vault':'violet',
    'Backup & Restore':'pink',
    'Cross-Screen Routing':'rose'
  });
...
export function renderSettingsScreen({stateService,currentDate,vaultAccessSession}) {
  ...
  );info.append(rows);const lowerGrid=node('section','settings-lower-grid');lowerGrid.append(defaults,security,backup,info);main.append(lowerGrid);
