...
import { detectForwardTimelineIssues, filterByTravelYears, sortItinerary, travelYearForDate, travelYearsForRange, availableTravelYears, findCurrentStay, findNextDestination, sameDayHandoffEntries } from './src_core_planning.js';
...
function hasAccommodation(state, itineraryId) { return (state.reservations || []).some(record => record.itineraryId === itineraryId && ['hotel','airbnb','accommodation'].includes(record.type) && record.status !== 'to-book' && !record.needsBudgetRepair); }
...
function displayCountryForEntry(entry = null) {
  if (!entry) return '';
...
function recordView(entry, state, journeyStartDate) {
  const travelYears=journeyStartDate?travelYearsForRange(entry.startDate,entry.endDate,journeyStartDate):[];
  const firstTravelYear=travelYears[0]??null, lastTravelYear=travelYears.at(-1)??firstTravelYear;
  return {
    id:entry.id,name:entry.name,country:displayCountryForEntry(entry),travelType:entry.travelType,startCity:entry.startCity||'',startCountry:entry.startCountry||'',
    startDate:entry.startDate,endDate:entry.endDate,displayDates:`${formatAUDate(entry.startDate)} – ${formatAUDate(entry.endDate)}`,
    days:stayDurationDays(entry.startDate,entry.endDate),travelYear:firstTravelYear,travelYearEnd:lastTravelYear,travelYearLabel:firstTravelYear?(lastTravelYear>firstTravelYear?`Years ${firstTravelYear}–${lastTravelYear}`:`Year ${firstTravelYear}`):null,
...
export function calculateForwardCoverage(entries, currentDate, months = 6, journeyStartDate = null, reservations = []) {
  const normalizedEntries=entries||[];
  const bookedFlightDates=new Set((reservations||[]).filter(record=>record.type==='flight'&&record.status!=='to-book'&&record.dateTime).map(record=>toISODate(record.dateTime)));
  ...
  let plannedDays=0,gapDays=0,overlapDays=0,flightTransitDays=0;
  const gaps=[]; const flightTransit=[]; let gapStart=null;
  const closeGap=endDay=>{if(gapStart!=null){gaps.push({type:'gap',startDate:dayISO(gapStart),endDate:dayISO(endDay),days:endDay-gapStart+1});gapStart=null;}};
  ...
  for(let day=current;day<=lastDay;day+=1){
    ...
    if(matches.length){
      plannedDays+=1;
      // Shared occupancy is overlap-free only for one unique two-stay handoff.
      // This keeps Forward Coverage aligned with planning and Budget routing:
      // two one-day stays or 3+ active stays on a date are ambiguous overlaps.
      if(matches.length > 1 && sameDayHandoffEntries(sorted, date).length !== 2) overlapDays+=1;
      closeGap(day-1);
    }
    else if(bookedFlightDates.has(date)){
      closeGap(day-1); flightTransitDays+=1; flightTransit.push({type:'flight-transit',startDate:date,endDate:date,days:1});
  ...
  const segments=[...stays,...gaps,...flightTransit].sort((a,b)=>String(a.startDate).localeCompare(String(b.startDate))||String(a.type).localeCompare(String(b.type)));
  return {
    startDate:startISO,endDate:horizonDays?dayISO(lastDay):startISO,horizonDays,plannedDays,gapDays,overlapDays,flightTransitDays,
    coveragePercent:horizonDays?Math.round(((plannedDays+flightTransitDays)/horizonDays)*100):0,segments
  };
}
...
function plannedCountries(entries){
  ...
  for(const entry of entries||[]){
    ...
    const sources=route ? [entry.startCountry||entry.country||''] : countryParts;
...
function plannedCountryDetails(entries){
  ...
  for(const entry of entries||[]){
    ...
    const countryParts=route
      ? []
      : [String(entry.country||'').trim()].filter(Boolean);
    const sources=(route ? [entry.startCountry||entry.country||''] : countryParts).filter(Boolean);
    for(const source of sources){
      const key=plannedCountryKey(source);
      if(!key)continue;
...
export function buildItineraryViewModel(state, currentDate, options = {}) {
  ...
  const forwardCoverage=calculateForwardCoverage(all,currentDate,options.coverageMonths??6,journeyStartDate,state.reservations||[]);
  // A completely untouched itinerary is a setup state, not a planning failure.
  // Do not count the entire forward horizon as one alarming gap until at least
  // one itinerary occurrence exists. Once planning has started, normal gap
  // detection resumes (including after the last completed stay).
  const coverageGaps=all.length ? (forwardCoverage.segments||[]).filter(segment=>segment.type==='gap') : [];
  ...
  journeyMap.availableYears=availableTravelYears(all,journeyStartDate,{minimum:30});
  ...
  const stopDetails=[
    ...futureOnly.map(entry=>({kind:'stay',id:entry.id,name:entry.name,country:displayCountryForEntry(entry),travelType:entry.travelType,startDate:entry.startDate,endDate:entry.endDate})),
