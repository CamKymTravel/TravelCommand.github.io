...
export function destinationLedger(state, itineraryId) {
  ...
  const expenses = (state.expenses || []).filter(record => record.budgetScope !== 'annual' && record.itineraryId === itineraryId && !record.needsBudgetRepair);
  const reservations = (state.reservations || []).filter(record => (record.budgetScope || (record.type === 'ticket' ? 'destination' : 'annual')) === 'destination' && record.itineraryId === itineraryId && record.status !== 'to-book' && !record.needsBudgetRepair);
