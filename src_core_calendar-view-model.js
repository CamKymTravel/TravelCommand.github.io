...
// on the premium dark material without becoming neon. Hashing still chooses a
// stable starting point, but collisions advance through this high-contrast
...
function paletteColourFromSeed(seed, probe = 0) {
  const index=(hash(seed)+Number(probe||0))%ITINERARY_COLOUR_PALETTE.length;
  return ITINERARY_COLOUR_PALETTE[index];
}

function itineraryIdentityKey(entry = {}) {
  const route=['cruise','motorhome','rv'].includes(String(entry.travelType||'').toLowerCase());
  const name=String(entry.name||'').normalize('NFC').trim().toLocaleLowerCase('en-AU');
  const country=String(route?(entry.startCountry||entry.country||''):(entry.country||entry.startCountry||'')).normalize('NFC').trim().toLocaleLowerCase('en-AU');
  return `${route?String(entry.travelType||'route').toLowerCase():'destination'}:${name}:${country}`;
}

export function buildItineraryColourMap(entries) {
  const used=new Set();
  const identityColours=new Map();
  const map=new Map();
  ...
  for(const entry of ordered){
    const identity=itineraryIdentityKey(entry);
    let colour=identityColours.get(identity);
    if(!colour){
      let probe=0; colour=paletteColourFromSeed(identity,probe);
      while(used.has(colour.color) && probe<ITINERARY_COLOUR_PALETTE.length){ probe+=1; colour=paletteColourFromSeed(identity,probe); }
      if(used.has(colour.color)){
        let generatedProbe=0; colour=colourFromSeed(identity,generatedProbe);
        while(used.has(colour.color) && generatedProbe<720){ generatedProbe+=1; colour=colourFromSeed(identity,generatedProbe); }
      }
      used.add(colour.color); identityColours.set(identity,colour);
    }
    map.set(entry.id,colour);
  }
  return map;
}

export function itineraryCalendarColour(entry) {
  return paletteColourFromSeed(itineraryIdentityKey(entry),0);
}
...
function itineraryEvent(entry, resolvedColour = null) {
  ...
  return {
    ...
    country:itineraryFlagCountry(entry),
...
function reservationEvent(record, itineraryById, colourByItineraryId) {
  ...
  return {
    ...
    country:itineraryFlagCountry(linked),
...
function allCalendarEvents(state) {
  ...
  const colourByItineraryId=buildItineraryColourMap(itineraryRecords);
