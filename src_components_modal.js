...
export function createModal({ title, body, actions = [], className = '', showCloseButton = false }) {
  ...
  if (isEditorModal || isExpandedModal || showCloseButton) {
    ...
    close.type = 'button';
    close.className = `tcc-modal-close${showCloseButton ? ' tcc-modal-close-labeled' : ''}`;
    const closeLabel = showCloseButton ? 'Close' : (isEditorModal ? 'Close editor without saving' : 'Close expanded view');
...
const EXPAND_INTERACTIVE = 'button, a, input, select, textarea, summary, [role="button"], [contenteditable="true"], [data-expand-ignore="true"]';
...
export function openExpandedCard({ host, source, title, tone = 'sky', body = null }) {
  ...
  // The declared widget tone is the authority. Expanded views must remain in
  // the exact same colour family as the collapsed widget; inferring from a
  // rendered border can accidentally pull in a neighbouring/legacy hue.
  const resolvedTone = fallbackTone;
  const content = document.createElement('div');
  content.className = `tcc-expanded-card-body tone-${resolvedTone}`;
  const snapshot = body || snapshotExpandedCard(source);
  // Expanded modals already carry the widget title in the modal chrome. When
  // either a cloned snapshot or a richer bodyBuilder begins with the exact same
  // heading, remove only that duplicate so every enlargement has one clear
  // title. Other internal section headings/status labels remain untouched.
  if (title && snapshot instanceof Element) {
    const normalise = value => String(value || '').replace(/\s+/g, ' ').trim().toLocaleLowerCase('en-AU');
    const firstHeading = snapshot.querySelector('h1, h2, h3');
    if (firstHeading && normalise(firstHeading.textContent) === normalise(title)) firstHeading.remove();
  }
