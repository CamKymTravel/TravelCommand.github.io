...
const RESERVATION_TYPES = new Set(['flight','train','cruise','rv','hotel','airbnb','accommodation','ticket']);
...
function normalizeNavigation(state, now) {
  ...
  // Legacy UI state may still point at the retired combined Accommodation tab.
  // Route that tab to Hotels without rewriting the underlying compatibility records.
  if (state.ui.reservationType === 'accommodation') state.ui.reservationType = 'hotel';
  ...
  // Home alerts are derived from live travel data. This UI-only list records
  // alerts Kym deliberately removed from the Home alert area without deleting
  // the underlying reservation, checklist, itinerary, Vault or Schengen data.
  const dismissedHomeAlertIds = Array.isArray(state.ui.dismissedHomeAlertIds) ? state.ui.dismissedHomeAlertIds : [];
  state.ui.dismissedHomeAlertIds = [...new Set(dismissedHomeAlertIds.filter(value => typeof value === 'string' && value.trim()).map(value => value.trim()))].slice(-250);
...
function repairCurrentPartialCoordinates(input) {
  // Current-generation state is otherwise validated strictly before the broad
  // compatibility normaliser runs. Coordinate pairs are the narrow exception:
  // older/current map edits can leave one side of an otherwise-valid pair
  // missing. The release contract requires known places to be repaired from the
  // offline gazetteer and unknown partial pairs to be cleared, never converted
  // to an accidental 0,0 point. Do not repair fully-present malformed values;
  // those remain corruption and must still enter Protected Recovery.
  const candidate = structuredClone(input);
  const isMissing = value => value == null || value === '';
  const validAxis = (value, min, max) => {
    if (typeof value === 'boolean' || (value != null && typeof value === 'object')) return false;
    const number = typeof value === 'number' ? value : (typeof value === 'string' && value.trim() !== '' ? Number(value) : NaN);
    return Number.isFinite(number) && number >= min && number <= max;
  };
  const repair = (record, placeName) => {
    if (!isPlainObject(record)) return record;
    const latMissing=isMissing(record.lat), longMissing=isMissing(record.long);
    if (latMissing === longMissing) return record;
    const presentValid = latMissing ? validAxis(record.long,-180,180) : validAxis(record.lat,-90,90);
    if (!presentValid) return record;
    const known=resolveOfflinePlace(placeName);
    return known ? { ...record, lat:known.lat, long:known.long } : { ...record, lat:null, long:null };
  };
  if (Array.isArray(candidate.itinerary)) {
    candidate.itinerary=candidate.itinerary.map(record=>repair(record, record?.travelType === 'standard' ? record?.name : record?.startCity));
  }
  if (Array.isArray(candidate.routePoints)) {
    candidate.routePoints=candidate.routePoints.map(record=>repair(record, record?.name));
  }
  return candidate;
}
...
function normalizeLegacyCost(state, record, kind) {
  ...
  const legacyAllocation = hadLegacyAllocation ? String(record.allocation || '').trim().toLowerCase() : null;
  const declaredScope = typeof record.budgetScope === 'string' ? record.budgetScope.trim().toLowerCase() : null;
  const legacyAnnualExpense = kind === 'expense' && legacyAllocation === 'annual' && String(record.category || '').trim().toLowerCase() === 'miscellaneous';
  const budgetScope = kind === 'expense'
    ? (declaredScope === 'annual' || legacyAnnualExpense ? 'annual' : 'destination')
    : (declaredScope === 'annual' || declaredScope === 'destination' ? declaredScope : (record.type === 'ticket' ? 'destination' : 'annual'));
  const legacyScopeConflict = kind === 'expense' && hadLegacyAllocation && !['destination','annual'].includes(legacyAllocation);
  const declaredScopeConflict = declaredScope != null && !['destination','annual'].includes(declaredScope);
  const retiredAnnualOutsideMisc = kind === 'expense' && legacyAllocation === 'annual' && !legacyAnnualExpense;
  ...
  if (kind === 'expense') {
    next.budgetScope = budgetScope;
    if (budgetScope === 'annual') next.itineraryId = null;
  ...
  if (kind === 'reservation') {
    ...
    next.bookingReference = textOr(next.bookingReference, '');
    next.notes = textOr(next.notes, '');
    next.status = normalizeReservationStatus(next.status);
    next.budgetScope = budgetScope === 'destination' ? 'destination' : 'annual';
  ...
  if (kind === 'expense' && budgetScope === 'annual') {
    // Direct Annual Budget expenses are deliberately simple: Miscellaneous
    // only, entered and stored in AUD, with no Destination Budget relationship.
    // A legacy annual-allocation record already has an AUD equivalent; use that
    // as the canonical entered amount so old backups preserve the same annual
    // spend while adopting the current AUD-only workflow.
    if (legacyAnnualExpense && next.audAmount > 0 && next.originalCurrency !== 'AUD') {
      next.originalCurrency = 'AUD';
      next.originalAmount = next.audAmount;
    }
    const annualAmountsValid = typeof next.originalAmount === 'number' && Number.isFinite(next.originalAmount) && next.originalAmount >= 0;
    if (annualAmountsValid && next.originalCurrency === 'AUD') next.audAmount = next.originalAmount;
    next.needsBudgetRepair = Boolean(
      next.needsBudgetRepair === true ||
      legacyScopeConflict ||
      declaredScopeConflict ||
      retiredAnnualOutsideMisc ||
      !date ||
      String(next.category || '').trim().toLowerCase() !== 'miscellaneous' ||
      next.originalCurrency !== 'AUD' ||
      !annualAmountsValid
    );
    return next;
  ...
  const handoff = date && matches.length === 2 ? sameDayHandoffCandidates(state.itinerary || [], date) : [];
  if (kind === 'reservation' && next.budgetScope === 'annual') {
    // Major bookings are Annual Budget costs. Keep a contextual stay link when
    // the date is unambiguous OR when a current record already points at one of
    // the two stays in a legal same-day handoff. saveReservationDraft preserves
    // that stable handoff relationship; hydration/restore must not erase it on
    // the next launch. Genuine overlaps and stand-alone transit days remain
    // unlinked and never require a Destination Budget merely to be retained.
    const contextualTarget = exact || (handoff.length === 2 && existingTarget && handoff.some(item => item.id === existingTarget.id) ? existingTarget : null);
    next.itineraryId = contextualTarget?.id || null;
    // Annual reservations own their saved canonical AUD amount. The optional
    // itineraryId is contextual only and does not prove how the AUD equivalent
    // was originally obtained. In particular, a later country correction can
    // make a formerly cross-currency manual booking equal the stay's new local
    // currency. Hydration must not reinterpret that historical AUD value or
    // falsely force the record into Budget Repair merely because of the new
    // contextual match. Validate the record itself, not inferred conversion
    // provenance that is not stored.
    const annualCurrency = String(next.originalCurrency || '').trim().toUpperCase();
    const canonicalAUDValid = annualCurrency !== 'AUD' || Math.abs(next.audAmount - next.originalAmount) <= 0.005;
    if (typeof next.audAmountManual !== 'boolean') {
      if (annualCurrency === 'AUD') next.audAmountManual = false;
      else if (!contextualTarget || !isDestinationBudgetUsable(contextualTarget) || annualCurrency !== String(contextualTarget.localCurrency || '').trim().toUpperCase()) next.audAmountManual = true;
      else {
        const expected = Math.round((next.originalAmount / Number(contextualTarget.fixedLocalPerAUD) + Number.EPSILON) * 100) / 100;
        next.audAmountManual = Math.abs(next.audAmount - expected) > 0.005;
      }
    }
    next.needsBudgetRepair = Boolean(next.needsBudgetRepair === true || declaredScopeConflict || !date || !validCurrency(next.originalCurrency) || !amountsValid || !canonicalAUDValid);
    return next;
  }
  // Current handoff-day costs can legitimately have two covering stays because
  // the user explicitly chose which stay ID owns the shared boundary date.
  // Preserve that stable relationship during hydration/migration; only genuine
  // overlaps, missing links, or links outside the legal handoff require repair.
  const resolvedTarget = exact || (handoff.length === 2 && existingTarget && handoff.some(item => item.id === existingTarget.id) ? existingTarget : null);
  const correctLink = Boolean(resolvedTarget && existingTarget && resolvedTarget.id === existingTarget.id);
  let automaticAUDValid = true;
  if (amountsValid && resolvedTarget && isDestinationBudgetUsable(resolvedTarget)) {
    const currency = String(next.originalCurrency || '').trim().toUpperCase();
    const stayCurrency = String(resolvedTarget.localCurrency || '').trim().toUpperCase();
    let expected = null;
    if (currency === 'AUD') expected = Math.round((next.originalAmount + Number.EPSILON) * 100) / 100;
    else if (currency === stayCurrency) expected = Math.round((next.originalAmount / Number(resolvedTarget.fixedLocalPerAUD) + Number.EPSILON) * 100) / 100;
    if (expected != null) automaticAUDValid = Math.abs(next.audAmount - expected) <= 0.005;
  }
  const wasDestination = !hadLegacyAllocation || legacyAllocation === 'destination';
  const repair = next.needsBudgetRepair === true || !wasDestination || legacyScopeConflict || declaredScopeConflict || !date || !correctLink || !isDestinationBudgetUsable(resolvedTarget) || !validCurrency(next.originalCurrency) || !amountsValid || !automaticAUDValid;
  next.budgetScope = kind === 'expense' ? 'destination' : next.budgetScope;
...
function normalizeCompatibleState(input, now) {
  ...
  {
    const source = isPlainObject(clone.settings.annualBudgetsAUD) ? clone.settings.annualBudgetsAUD : {};
    const normalized = {};
    for (const [year,value] of Object.entries(source)) if (/^\d{4}$/.test(year)) normalized[year] = normalizeNumber(value,{min:0,missing:0});
    if (!Object.keys(normalized).length && clone.settings.journeyStartDate && Number(clone.settings.annualBudgetAUD||0) >= 0) normalized[String(clone.settings.journeyStartDate).slice(0,4)] = Number(clone.settings.annualBudgetAUD||0);
    clone.settings.annualBudgetsAUD = normalized;
  }
  ...
  clone.itinerary = clone.itinerary.map(record => {
    ...
    return {
      ...
      notes:textOr(record.notes ?? record.planningNote ?? record.note, ''),
      lat:normalizeNumber(record.lat, { min:-90, max:90, missing:null }),
      long:normalizeNumber(record.long, { min:-180, max:180, missing:null })
    };
  }).map(record => {
    if (!isPlainObject(record) || (record.lat != null && record.long != null)) return record;
    const known=resolveOfflinePlace(record.travelType === 'standard' ? record.name : record.startCity);
    return known ? { ...record, lat:known.lat, long:known.long } : { ...record, lat:null, long:null };
  });
  ...
  clone.routePoints = clone.routePoints.map(record => {
    ...
    if (next.lat == null || next.long == null) {
      const known=resolveOfflinePlace(next.name);
      if(known){ next.lat=known.lat; next.long=known.long; }
      else { next.lat=null; next.long=null; }
    }
    return next;
  });
...
export function migrateState(input, { now = new Date().toISOString() } = {}) {
  ...
  if (version === SCHEMA_VERSION && isStrictPersistedGeneration(input?.meta?.appVersion)) {
    ...
    // a clone before full validation. A narrowly-scoped coordinate pre-repair
    // handles valid partial map pairs without weakening any other strict field.
    migrationInput = repairCurrentPartialCoordinates(input);
    validateCurrentStateAllowingNavigationRepair(migrationInput, { now });
  }
  if (version === 0 || version === 1 || version === SCHEMA_VERSION) return normalizeCompatibleState(migrationInput, now);
