...
export function validateState(state) {
  ...
  if (state.settings.annualBudgetsAUD != null) {
    if (!isObject(state.settings.annualBudgetsAUD) || Array.isArray(state.settings.annualBudgetsAUD)) throw new Error('Annual Budgets by year must be an object');
    for (const [year, value] of Object.entries(state.settings.annualBudgetsAUD)) { if (!/^\d{4}$/.test(year)) throw new Error('Annual Budget year key is invalid'); strictNumber(value, `Annual budget ${year}`, { min:0 }); }
  }
...
export function validateItinerary(entry) {
  ...
  optionalText(entry.notes, 'Itinerary planning note');
...
export function validateRoutePoint(point) {
  ...
  normalizeCoordinates(point.lat, point.long, { allowEmpty:true });
  return true;
}
...
export function validateExpense(expense) {
  ...
  if (!['destination','annual'].includes(budgetScope)) throw new Error('Expense budget scope is invalid');
  optionalId(expense.itineraryId, 'Expense itinerary');
  if (!expense.needsBudgetRepair && budgetScope === 'destination' && !expense.itineraryId) throw new Error('Destination expense requires an itinerary');
  if (!expense.needsBudgetRepair && budgetScope === 'annual' && expense.itineraryId != null) throw new Error('Annual Budget expense cannot be linked to a Destination Budget');
  if (!expense.needsBudgetRepair && budgetScope === 'annual' && expense.category !== 'miscellaneous') throw new Error('Only Miscellaneous expenses can be charged directly to the Annual Budget');
  ...
  if (!expense.needsBudgetRepair && budgetScope === 'annual') {
    if (expense.originalCurrency !== 'AUD') throw new Error('Annual Budget expenses must be entered in AUD');
    if (Math.abs(Number(expense.audAmount) - Number(expense.originalAmount)) > 0.005) throw new Error('Annual Budget expense AUD amount must match the entered amount');
  }
  return true;
}
...
export function validateReservation(reservation) {
  ...
  const budgetScope = reservation.budgetScope == null ? (reservation.type === 'ticket' ? 'destination' : 'annual') : reservation.budgetScope;
  if (!['annual','destination'].includes(budgetScope)) throw new Error('Invalid reservation budget scope');
  ...
  optionalText(reservation.bookingReference, 'Reservation booking reference');
  ...
  if (!reservation.needsBudgetRepair && budgetScope === 'destination' && !reservation.itineraryId) throw new Error('Destination reservation requires an itinerary');
  ...
  if (reservation.audAmountManual != null && typeof reservation.audAmountManual !== 'boolean') throw new Error('Reservation AUD conversion mode is invalid');
...
function validateUI(ui) {
  ...
  if (!Array.isArray(ui.dismissedHomeAlertIds) || ui.dismissedHomeAlertIds.some(value => typeof value !== 'string' || !value.trim())) throw new Error('Invalid dismissed Home alerts');
  if (new Set(ui.dismissedHomeAlertIds).size !== ui.dismissedHomeAlertIds.length) throw new Error('Dismissed Home alerts must be unique');
  if (ui.dismissedHomeAlertIds.length > 250) throw new Error('Too many dismissed Home alerts');
