...
function compactTimeline(state,currentDate){
  ...
  for(const e of entries){ const row=node('div','home-mini-row home-timeline-row'); const icon=node('span','home-timeline-icon'); icon.append(createLineIcon(e.travelType==='cruise'?'cruise':e.travelType==='motorhome'?'rv':'history')); const copy=node('span','home-timeline-copy'); copy.append(node('strong','',e.name),node('small','',`${formatAUDate(e.startDate)} – ${formatAUDate(e.endDate)}`)); const status=String(e.startDate)<=String(currentDate)&&String(e.endDate)>=String(currentDate)?'In progress':'Upcoming'; row.append(icon,copy,node('span',`home-timeline-status ${status==='In progress'?'is-current':''}`,status)); row.setAttribute('aria-hidden','true'); list.append(row); }
...
function buildTimelineExpanded(state,currentDate){
  ...
  section.append(homeExpandedPagedList(entries,{emptyText:'No entries yet',itemLabel:'segments',renderItem:e=>{
    const active=String(e.startDate)<=String(currentDate)&&String(e.endDate)>=String(currentDate);const row=node('div','home-expanded-list-row timeline-row');
  ...
  body.append(node('p','home-expanded-callout','Full forward Trip Timeline · read-only.'),section);return body;
}
