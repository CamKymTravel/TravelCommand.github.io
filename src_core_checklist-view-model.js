...
import { findCurrentStay, findNextDestination, sameDayHandoffEntries } from './src_core_planning.js';
...
function displayCountryForStay(stay){
  const route=['motorhome','cruise','rv'].includes(String(stay?.travelType||'').toLowerCase());
...
export function deriveChecklistMoveContext(itinerary,currentDate){
  const today=toISODate(currentDate);const records=itinerary||[];const current=findCurrentStay(records,today);const next=findNextDestination(records,today);
  const handoff=sameDayHandoffEntries(records,today);
  // Checklist is move-oriented. On a legal shared handoff date, the arriving
  // stay is the destination Cameron & Kym are moving toward even though Home
  // may still present the leaving stay as current without time-of-day data.
  if(handoff.length===2)return{destination:handoff[1],stage:'travel-day'};
...
export function buildChecklistViewModel(state,currentDate,options={}){
  ...
  const his=sortItems(optionalItems.filter(item=>item.stage===activeStage&&item.owner==='cameron'));const hers=sortItems(optionalItems.filter(item=>item.stage===activeStage&&item.owner==='kym'));const sharedOptional=sortItems(optionalItems.filter(item=>item.stage===activeStage&&item.owner==='both'));const sharedPermanentOptional=sharedOptional.filter(item=>item.listType==='permanent');const sharedDestinationOptional=sharedOptional.filter(item=>item.listType==='destination');
