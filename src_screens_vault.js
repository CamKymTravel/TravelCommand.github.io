...
function formatLocalActivityDate(value) {
  if (!value) return '—';
  const instant = new Date(value);
  if (Number.isNaN(instant.valueOf())) return '—';
  return formatAUDate(localISODate(instant));
}
...
function openVaultRecordEditor({ stateService, host, recordId = null, initialCategory = 'passport', access = null, editorTone = null }) {
  ...
  modal = createModal({ title:existing?'Edit Vault Record':'Add Vault Record', body, actions, className:`tcc-editor-modal tcc-vault-editor-modal tone-${resolvedTone}` });
...
function openAttachmentPicker({ stateService, host, record, editorTone = null }) {
  const resolvedTone=editorTone || VAULT_TONES[record?.category] || 'blue';
  ...
  input.addEventListener('change', async () => {
    try {
      ...
      const modal=createModal({title:'Add Screenshots',body,className:`tcc-editor-modal tcc-vault-editor-modal tone-${resolvedTone}`,actions:[
...
function openStreamingEditor({ stateService, host, recordId = null, editorTone = null, presetService = '' }) {
  ...
  const actions=[];
  if(existing) actions.push({label:'Delete',kind:'danger',onClick:dialog=>confirmDestructive({title:'Delete Streaming record',message:`Delete ${existing.service} · ${existing.owner || 'Shared'}${existing.username ? ` · ${existing.username}` : ''}? This cannot be undone.`,tone:resolvedTone,onConfirm:()=>{stateService.commit(draft=>deleteStreamingDraft(draft,existing.id));if(dialog.isConnected&&dialog.open)dialog.close();}})});
...
function openEmailEditor({ stateService, host, recordId = null, editorTone = 'gold' }) {
  ...
  modalHost(host,createModal({title:existing?'Edit Protected Email':'Add Protected Email',body,actions,className:`tcc-editor-modal tcc-vault-editor-modal tone-${resolvedTone}`}));
}
...
function renderLocked(main, stateService, access, requestRender) {
  ...
  hero.addEventListener('click',()=>{
    ...
    if(taps>=3){
      ...
      openUnlockDialog({stateService,host:main,access,requestRender});
      return;
    }
...
function vaultExpandedStat(label,value,sub='',tone=''){
  ...
  card.append(node('span','',label),node('strong','',String(value)),node('small','',sub));
  return card;
}

function vaultSummaryExpandedBody(state,{active,expiring,expired}){
  const body=node('div','home-expanded-dashboard vault-overview-expanded');
  const stats=node('div','home-expanded-stats home-expanded-stats-four');
  stats.append(
    vaultExpandedStat('TOTAL RECORDS',state.vault.length,`${state.attachments.length} screenshot attachments`,'teal'),
    vaultExpandedStat('VALID / ACTIVE',active,'records not past their expiry/end date','green'),
    vaultExpandedStat('EXPIRY WATCH',expiring.length,'records expiring within 180 days','gold'),
    vaultExpandedStat('EXPIRED',expired.length,'records requiring review','red')
  );
  body.append(stats);
  const section=node('section','home-expanded-section');section.append(node('h3','','RECORDS BY TYPE'));
  const list=node('div','home-expanded-list');
  for(const category of VAULT_CATEGORIES){
    const records=state.vault.filter(record=>record.category===category);
    const attachmentCount=state.attachments.filter(item=>records.some(record=>record.id===item.vaultRecordId)).length;
    const row=node('div','home-expanded-list-row');
    const tag=node('span','home-expanded-priority',String(records.length));
    const copy=node('span','home-expanded-list-copy');copy.append(node('strong','',VAULT_CATEGORY_LABELS[category]),node('small','',`${records.length} ${records.length===1?'record':'records'} · ${attachmentCount} screenshot${attachmentCount===1?'':'s'}`));
    row.append(tag,copy,node('b','',records.length?'STORED':'EMPTY'));list.append(row);
  }
  section.append(list);body.append(section);
  body.append(node('p','home-expanded-callout','Vault records and screenshots are stored locally on this iPad. This overview enlarges the complete document position without exposing protected fields on the overview screen.'));
  return body;
}

function vaultExpiryExpandedBody(expiring,daysUntil){
  const body=node('div','home-expanded-dashboard vault-expiry-expanded');
  const stats=node('div','home-expanded-stats home-expanded-stats-four');
  ...
  const list=node('div','home-expanded-list');
  if(!expiring.length)list.append(node('p','home-expanded-empty','No records expire within 180 days.'));
  for(const record of expiring){
    const days=daysUntil(record.expiryDate);const row=node('div','home-expanded-list-row');
    const tag=node('span','home-expanded-priority',days<=30?'URGENT':days<=90?'WATCH':'AHEAD');
    const copy=node('span','home-expanded-list-copy');copy.append(node('strong','',record.title),node('small','',[VAULT_CATEGORY_LABELS[record.category],record.owner||'Shared',`Expires ${formatAUDate(record.expiryDate)}`].filter(Boolean).join(' · ')));
    row.append(tag,copy,node('b','',days===0?'TODAY':`${days} DAYS`));list.append(row);
  }
...
function vaultActivityExpandedBody(state){
  const body=node('div','home-expanded-dashboard vault-activity-expanded');
  const vaultById=new Map(state.vault.map(record=>[record.id,record]));
  const activity=[
    ...state.vault.map(record=>({kind:'RECORD',title:record.title,subtitle:[VAULT_CATEGORY_LABELS[record.category],record.owner||'Shared'].filter(Boolean).join(' · '),modifiedAt:record.modifiedAt})),
...
const STREAMING_ICON_ASSETS = Object.freeze({
  netflix:'./brand-stream-netflix.png', disney:'./brand-stream-disney.png', prime:'./brand-stream-prime.png', apple:'./brand-stream-apple.png',
  max:'./brand-stream-max.png', paramount:'./brand-stream-paramount.png', stan:'./brand-stream-stan.png', binge:'./brand-stream-binge.png',
  kayo:'./brand-stream-kayo.png', youtube:'./brand-stream-youtube.png', afl:'./brand-stream-afl.png', nfl:'./brand-stream-nfl.png',
  iview:'./brand-stream-iview.png', sbs:'./brand-stream-sbs.png', seven:'./brand-stream-seven.png', nine:'./brand-stream-nine.png'
});
...
function streamingServiceMark(service) {
  ...
  const mark=node('span',`vault-streaming-mark vault-streaming-mark-${slug}`);
  const asset=STREAMING_ICON_ASSETS[slug];
  if(asset){
    const img=document.createElement('img');img.src=asset;img.alt='';img.loading='eager';img.decoding='async';mark.append(img);
