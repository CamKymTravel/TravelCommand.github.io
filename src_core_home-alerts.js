...
export function buildHomeAlerts(state,currentDate){
  ...
  for(const reservation of state.reservations||[]){if(reservation.status!=='to-book'||!reservation.dateTime)continue;const date=toISODate(reservation.dateTime);const days=daysUntil(date,today);const due=formatAUDate(date);const time=enteredTime(reservation.dateTime);const planned=[due,time].filter(Boolean).join(' · ');if(days<0)alerts.push(alert(`to-book:${reservation.id}`,'To Book Overdue',`${reservation.title} · booking overdue · ${planned}.`,'critical',{screen:'reservations',collection:'reservations',id:reservation.id},date));else{const timing=days===0?'today':`in ${days} day${days===1?'':'s'}`;alerts.push(alert(`to-book:${reservation.id}`,'To Book',`${reservation.title} · to book · ${planned} · ${timing}.`,days<=14?'high':'medium',{screen:'reservations',collection:'reservations',id:reservation.id},date));}}
  ...
  const coverage=calculateForwardCoverage(state.itinerary||[],today,3,journeyStart,state.reservations||[]);
  // An untouched/empty itinerary is already represented by the Home setup prompt.
  // Do not turn the journey start date into a misleading 3-month Missing Coverage
  // warning until at least one itinerary stay exists. Once planning has begun,
  // genuine uncovered days remain actionable and are still surfaced here.
  if(sorted.length>0&&coverage.gapDays>0)alerts.push(alert('coverage:3m','Missing Coverage',`${coverage.gapDays} uncovered day${coverage.gapDays===1?'':'s'} in the next 3 months.`,'high',{screen:'itinerary',collection:null,id:null},coverage.segments.find(s=>s.type==='gap')?.startDate||null));
  ...
  for(const stay of state.itinerary||[]){if(toISODate(stay.endDate)<today)continue;if(!isDestinationBudgetUsable(stay))alerts.push(alert(`budget:${stay.id}`,'Destination Budget Needs Setup',`${stay.name} · ${formatAUDate(stay.startDate)} – ${formatAUDate(stay.endDate)} needs an AUD budget and fixed exchange rate. Its local currency comes automatically from the itinerary.`,'high',{screen:'budget',collection:'itinerary',id:stay.id},stay.startDate));}
  const repairs=[...(state.expenses||[]).map(r=>['expenses',r]),...(state.reservations||[]).map(r=>['reservations',r])].filter(([,r])=>r.needsBudgetRepair);
  for(const [collection,record] of repairs){const repairDate=collection==='expenses'?record.date:record.dateTime;const kind=collection==='expenses'?'expense':'reservation';const fallback=collection==='expenses'?String(record.category||'Expense').replace(/-/g,' ').replace(/\b\w/g,c=>c.toUpperCase()):'Reservation';const subject=String(collection==='expenses'?(record.description||fallback):(record.title||fallback)).trim()||fallback;const dateLabel=repairDate?formatAUDate(repairDate):'date not set';const title=collection==='expenses'?'Expense Routing Repair':'Destination Budget Repair';const message=collection==='expenses'?`${subject} · ${dateLabel} needs expense routing repair. Amount/currency cannot be confirmed until the record is repaired.`:`${subject} · ${dateLabel} needs Destination Budget repair. Amount/currency cannot be confirmed until the record is repaired.`;alerts.push(alert(`repair:${collection}:${record.id}`,title,message,'critical',{screen:collection==='expenses'?'budget':'reservations',collection,id:record.id},repairDate||null));}

  // The Vault is protected even when Home is visible. Keep expiry warnings
  // operational, but never leak the protected record title, owner or category
  // before the user completes the Vault unlock flow. The exact record pointer is
  // retained internally so opening the alert can still land on the right record
  // after unlock.
  for(const record of state.vault||[]){if(!record.expiryDate)continue;const days=daysUntil(record.expiryDate,today);if(days>180)continue;const expiry=formatAUDate(record.expiryDate);const timing=days===0?'today':`in ${days} day${days===1?'':'s'}`;alerts.push(alert(`vault-expiry:${record.id}`,'Vault Expiry',days<0?`A protected Vault record expired ${expiry}.`:`A protected Vault record expires ${expiry} · ${timing}.`,days<0?'critical':days<=14?'high':days<=60?'medium':'low',{screen:'vault',collection:'vault',id:record.id},record.expiryDate));}
