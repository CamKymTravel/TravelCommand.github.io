...
export function buildHomeAlerts(state,currentDate){
  ...
  for(const [collection,record] of repairs){const repairDate=collection==='expenses'?record.date:record.dateTime;const kind=collection==='expenses'?'expense':'reservation';const fallback=collection==='expenses'?String(record.category||'Expense').replace(/-/g,' ').replace(/\b\w/g,c=>c.toUpperCase()):'Reservation';const subject=String(collection==='expenses'?(record.description||fallback):(record.title||fallback)).trim()||fallback;const dateLabel=repairDate?formatAUDate(repairDate):'date not set';const title=collection==='expenses'?'Expense Routing Repair':'Destination Budget Repair';const message=collection==='expenses'?`${subject} · ${dateLabel} needs expense routing repair. Amount/currency remain unverified until the record is repaired.`:`${subject} · ${dateLabel} needs Destination Budget repair. Amount/currency remain unverified until the record is repaired.`;alerts.push(alert(`repair:${collection}:${record.id}`,title,message,'critical',{screen:collection==='expenses'?'budget':'reservations',collection,id:record.id},repairDate||null));}
